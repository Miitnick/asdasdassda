...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            wait: '[ESPERE] Em andamento⏳ aguarde um momento. ',
            error: {
                St: '[❗] Envie a imagem com a legenda *!sticker * ou a marque a imagem que foi enviada',
                ...
                Ki: '[❗] O bot não pode expulsar um administrador de grupo!',
        ...
        const ownerNumber = ["6285521992491007@c.us","55xxxxx"] // replace with your whatsapp number
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    client.reply(from, '[ESPERE] Em andamento ⏳ aguarde ± 1 min!', id)
        ...
        case '!donate':
            ...
            break
        case '!tts':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!tts [id, en, jp, ar] [texto]*, por exemplo *! Tts en hello all *')
	    ...
	    const ttsJp = require('node-gtts')('ja')
	        ...
	        if (dataBhs == 'id') {
                ...
                client.reply(from, 'Insira os codigo do idioma: [id] para indonésio, [en] para inglês, [jp] para japonês e [ar] para árabe', id)
            }
            break
        case '!nulis':
            ...
            break
        case '!ytmp3':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!Ytmp3 [linkYt] *, por exemplo, envie o comando * !Readme *')
            ...
            break
        case '!ytmp4':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!Ytmp4 [linkYt] *, por exemplo, envie o comando * !Readme * ')
            ...
            break
        case '!wiki':
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!wiki [consulta]*\nExemplo: *!wiki asu *', id)
            ...
            break
        case '!cuaca':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!cuaca [local] * \nExemplo: *!cuaca tangerang', id)
            ...
            break
        case '!fb':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!Fb [linkFb] * por exemplo, envie o comando *!Readme *', id)
        ...
        case '!creator':
            ...
            break
        case '!ig':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!ig [linkIg] * por exemplo, envie o comando *!readme*')
        ...
        case '!nsfwmenu':
            ...
            break
        case '!igstalk':
            if (args.length === 1)  return client.reply(from, 'Enviar pedidos *!igStalk @username*\nExemplo *!igStalk @username*', id)
        ...
        case '!infogempa':
            ...
            break
        case '!anime':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!anime [consulta] * \ Exemplo: *!Anime darling in the franxx*', id)
            ...
            if (args.length === 2) {
                ...
                if (cek === true)  {
                    try {
                        ...
                        client.reply(from, '[❗] Algo deu errado, talvez o código nuclear esteja errado', id)
                    }
                } else {
                ...
                await BrainlySearch(tanya.split('.')[0],Number(jum), function(res){
                    res.forEach(x=>{
                        if (x.jawaban.fotoJawaban.length == 0) {
                            ...
                            client.reply(from, `➸ *Questão* : ${x.pertanyaan}\n\n➸ *Resposta* : ${x.jawaban.judulJawaban}\n\n➸ *Link foto resposta* : ${x.jawaban.fotoJawaban.join('\n')}`, id)
                        }
                    })
                })
            } else {
        ...
        case '!quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                client.reply(from, 'Usage: \n!quotemaker |texto|marca dágua|tema\n\nEx :\n!quotemaker |este exemplo | bicit | aleatório', id)
            }
            break
        case '!linkgroup':
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot for administrador', id)
            if (isGroupMsg) {
            ...
            break
        case '!bc':
            if (!isOwner) return client.reply(from, 'Este comando é apenas para bots Proprietários!', id)
            let msg = body.slice(4)
            const chatz = await client.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendText(ids, `[ RORSCHACH BOT Broadcast ]\n\n${msg}`)
            }
        ...
        case '!kickall':
            ...
            if (!isGroupOwner) return client.reply(from, 'Este comando só pode ser usado pelo proprietario do grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot se tornar administrador', id)
        ...
        case '!leaveall':
            ...
            for (let gclist of allGroups) {
                await client.sendText(gclist.contact.id, `Desculpe, o bot está limpando, o chat total está ativo : ${allChats.length}`)
                await client.leaveGroup(gclist.contact.id)
            }
            client.reply(from, 'Succes leave all group!', id)
            break
        case '!clearall':
            if (!isOwner) return client.reply(from, 'Este comando é apenas para o proprietario do bot', id)
        ...
        case '!add':
            ...
            if (args.length === 1) return client.reply(from, 'Para usar este recurso, envie um comando *!add* 628xxxxx', id)
        ...
        case '!kick':
            ...
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este comando, envie um comando*!kick* @tagmember', id)
            await client.sendText(from, `Pedidos recebidos, emitidos:\n${mentionedJidList.join('\n')}`)
        ...
        case '!promote':
            ...
            if (mentionedJidList.length === 0) return client.reply(from, 'para usar este recurso, envie o comando *!promote* @tagmember', id)
            ...
            await client.sendTextWithMentions(from, `Pedidos aceitos, adicionados @${mentionedJidList[0]} sebagai admin.`)
            break
        case '!demote':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores de grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado quando o bot é um administrador', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este recurso, envie um comando *!demote* @tagadmin', id)
            if (mentionedJidList.length >= 2) return client.reply(from, 'Desculpe, este comando só pode ser usado em 1 pessoa.', id)
            ...
            await client.sendTextWithMentions(from, `Pedidos recebidos, excluir posições @${mentionedJidList[0]}.`)
            break
        case '!join':
            //return client.reply(from, 'Se você quiser convidar o bot para o seu grupo, dê permissão para wa.me/5521992491007', id)
            if (args.length < 2) return client.reply(from, 'Enviar pedidos *!join linkgroup key*\n\nEx:\n!join https://chat.whatsapp.com/blablablablablabla', id)
            const link = args[1]
            const key = args[2]
            ...
            if (!isLink) return client.reply(from, 'Este é um link? 👊🤬', id)
            ...
            if (check.status === 200) {
                await client.joinGroupViaLink(link).then(() => client.reply(from, 'O bot chegará em breve!'))
            } else {
        ...
        case '!delete':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores de grupo', id)
            if (!quotedMsg) return client.reply(from, 'Errado!!, envie o comando *!delete [marque a mensagem do bot] *', id)
        ...
        case '!getses':
            ...
            break
        case '!lirik':
            if (args.length == 1) return client.reply(from, 'Send command *!irik [opcional] *, exemplo *!irik Im not a doll *', id)
            const lagu = body.slice(7)
            const lirik = await liriklagu(lagu)
            client.reply(from, lirik, id)
            break
        case '!chord':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!chord [query] *, por exemplo *!chord não é um fantoche*', id)
        ...
        case '!listchannel':
            ...
            break
        case '!jadwaltv':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!jadwaltv [canal] *', id)
        ...
        case '!randomhentai':
            if (isGroupMsg) {
                if (!isNsfw) return client.reply(from, 'Comando NSFW não ativado neste grupo!', id)
